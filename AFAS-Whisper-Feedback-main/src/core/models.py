...
class Grammar(Base):
    """
    Model storing grammar quality metrics.

    Measures grammar accuracy based on the ASR transcript generated by Whisper.
    The grammar checker analyzes the transcript text and calculates the number
    of grammatical errors, the ratio of sentences containing errors, and the
    error rate per word.
    ...
    Attributes:
        ...
        total_errors: Total number of grammar errors detected
        error_rate: Number of grammar errors divided by total words
        total_sentences: Number of detected sentences in the transcript
...
class Feedback(Base):
    ...
    grammar_id = Column(Integer, nullable=True)
